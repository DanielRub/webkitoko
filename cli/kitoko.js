...
// Pass in webkitoko as a blueprint module.
// User passes in blueprints flag but without webkitoko :> append webkitoko
if (!process.argv.includes('webkitoko') && process.argv.includes('--blueprints')) {
    for (let i = 0; i < process.argv.length; i++) {
        if (process.argv[i] === '--blueprints') {
            process.argv[i + 1] = `${process.argv[i + 1].split(',')},webkitoko`;
        }
    }
    // User passes in blueprint flag but without webkitoko :> append webkitoko
} else if (!process.argv.includes('webkitoko') && process.argv.includes('--blueprint')) {
    for (let i = 0; i < process.argv.length; i++) {
        if (process.argv[i] === '--blueprint') {
            process.argv[i] = '--blueprints';
            process.argv[i + 1] = `${process.argv[i + 1]},webkitoko`;
        }
    }
    // User do not pass in blueprints or blueprint flag but without webkitoko :> append webkitoko
} else if (!process.argv.includes('webkitoko') && !process.argv.includes('--blueprint') && !process.argv.includes('--blueprints')) {
    process.argv.push('--blueprints');
    process.argv.push('webkitoko');
}
    ...
    if (preferLocal) {
        try {
            ...
            if (__dirname !== path.dirname(localCLI)) {
                ...
                logger.info("Webkitoko is loading a JHipster version installed locally in current project's node_modules");
                require(localCLI);
                return;
            }
        } catch (e) {
